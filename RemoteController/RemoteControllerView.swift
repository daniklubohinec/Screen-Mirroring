...
struct RemoteControlView: View {
    let actions: RemoteControlActions
    ...
    var body: some View {
        VStack(spacing: 20) {
            Spacer()
            Text("TV Remote")
                .font(.init(UIFont(name: "Inter-Bold", size: 32)!))
        ...
        .sheet(isPresented: $isVSelectionPresented) {
            RemoteConnectorInitView(isPresented: $isVSelectionPresented, onTVSelected: { name, ipAddress in
